...
class RAGSystem(RAGSystemInterface):
    ...
    def _perform_health_checks(self) -> None:
        ...
        if self.vectorizer:
            try:
                ...
                self.logger.warning(f"Vectorizer health check failed: {e}")
        ...
        if self.vector_store:
            try:
                ...
                self.logger.warning(f"Vector store health check failed: {e}")
        ...
        if self.response_generator:
            try:
                ...
                self.logger.warning(f"Response generator health check failed: {e}")
    ...
    def _retrieve_relevant_faqs(self, processed_query: ProcessedQuery, context) -> List[FAQEntry]:
        """Retrieve relevant FAQs with multiple fallback strategies."""
        retrieved_faqs = []
        
        # Strategy 1: Vector similarity search (preferred)
        if self.vectorizer and self.vector_store:
            try:
                self.logger.debug("Attempting vector similarity search...")
                ...
                self.logger.info(f"Vector search found {len(retrieved_faqs)} relevant FAQs")
                
            except Exception as e:
                self.logger.warning(f"Vector similarity search failed: {e}")
