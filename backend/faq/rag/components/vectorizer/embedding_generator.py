...
class FAQEmbeddingGenerator:
    ...
    def __init__(self):
        """Initialize the embedding generator with Gemini service."""
        try:
            self.gemini_service = GeminiEmbeddingService()
            self.embedding_dimension = self.gemini_service.get_embedding_dimension()
    ...
    def validate_embeddings(self, faqs: List[FAQEntry]) -> ValidationResult:
        ...
        for faq in faqs:
            ...
            if not self.gemini_service.validate_embedding(faq.embedding):
    ...
    def _prepare_text_components(self, faq: FAQEntry) -> Dict[str, str]:
        ...
        Args:
            faq: FAQ entry to process
            
        Returns:
            Dictionary of component names to cleaned text
        """
        components = {}
        
        # Clean and prepare question
        if faq.question:
            components['question'] = clean_text(faq.question)
        
        # Clean and prepare answer
        if faq.answer:
            components['answer'] = clean_text(faq.answer)
        
        # Prepare keywords
        if faq.keywords:
            components['keywords'] = ' '.join(faq.keywords)
        ...
        components['composite'] = ' '.join(composite_parts)
    ...
    def _create_composite_embedding(self, embeddings: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        Uses weighted averaging to combine question, answer, and keyword embeddings
        into a single representative embedding.
        
        Args:
            embeddings: Dictionary of component embeddings
            
        Returns:
            Composite embedding vector
        """
        if 'composite' in embeddings and self.gemini_service.validate_embedding(embeddings['composite']):
            # Use composite embedding if available and valid
        ...
        if 'question' in embeddings and self.gemini_service.validate_embedding(embeddings['question']):
            valid_embeddings.append(embeddings['question'])
            weights.append(0.5)
        
        # Answer gets medium weight (0.4)
        if 'answer' in embeddings and self.gemini_service.validate_embedding(embeddings['answer']):
            valid_embeddings.append(embeddings['answer'])
            weights.append(0.4)
        
        # Keywords get lower weight (0.1)
        if 'keywords' in embeddings and self.gemini_service.validate_embedding(embeddings['keywords']):
    ...
    def _validate_embedding_quality(self, embedding: np.ndarray, faq: FAQEntry) -> bool:
        ...
        Args:
            ...
            faq: Original FAQ entry for context
            
        Returns:
            True if embedding quality is acceptable
        """
        if not self.gemini_service.validate_embedding(embedding):
            return False
    ...
    def get_embedding_stats(self, faqs: List[FAQEntry]) -> Dict[str, Any]:
        ...
        for faq in faqs:
            if faq.embedding is not None and self.gemini_service.validate_embedding(faq.embedding):
