...
class VectorStore(VectorStoreInterface):
    ...
    def search_with_filters(self, query_vector: np.ndarray, threshold: float = 0.7, 
                           top_k: int = 10, category_filter: Optional[str] = None,
        ...
        Args:
            ...
            threshold: Minimum similarity threshold (0.0 to 1.0)
            top_k: Maximum number of results to return
            category_filter: Filter by FAQ category
            confidence_filter: Minimum confidence score for FAQ entries
            keyword_filter: Filter by keywords (must contain at least one)
            
        Returns:
            List of filtered similarity matches sorted by score (descending)
        """
        start_time = datetime.now()
        ...
        with self._lock:
            ...
            matches = self._compute_similarities(query_vector, threshold, top_k * 2)  # Get more for filtering
            ...
            for match in matches:
                ...
                # Category filter
                if category_filter and faq_entry.category != category_filter:
                    continue
