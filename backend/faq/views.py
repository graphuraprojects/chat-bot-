...
@csrf_exempt
# ai_faq_search removed (deprecated)
@csrf_exempt
@require_http_methods(["POST"])
def chatbot_response(request):
    """
    Generate deterministic chatbot responses.
    Flow:
    1. User Query -> Gemini (Spelling/Grammar Correction)
    2. Corrected Query -> FAQ Matcher (DB Search)
    3. Match Found -> Return DB Answer (Strict)
    4. No Match -> Gemini Fallback (Polite decline)
    """
    try:
        payload = json.loads(request.body.decode("utf-8")
                             ) if request.body else {}
        ...
        if not user_name and user and not user.name.startswith("Anonymous User"):
            user_name = user.name

        # 1. Spelling/Grammar Correction
        # from .gemini_service import gemini_service
        corrected_query = message # Temporarily bypass Gemini
        # corrected_query = gemini_service.correct_query_spelling(message)
        logger.info(f"Original: {message} | Corrected: {corrected_query}")

        # 2. Database Matching (Using corrected query)
        from .faq_matcher import faq_matcher
        matches = faq_matcher.find_best_matches(
            query=corrected_query,
            max_results=1,
            min_confidence=0.3  # Lowered to catch word variations
        )
        ...
        if matches:
            ...
            # 4. Fallback
            source = "fallback"
            response_text = "I'm sorry, I don't have an answer for that." # Temporarily bypass Gemini
            # response_text = gemini_service.generate_fallback_response(
            #     message, user_name)
            confidence = 0.0

        # Log Interaction (always log now)
